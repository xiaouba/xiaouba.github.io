...
- `./publish.sh --check --file "content/posts/文章名.md"`: validate front matter before publishing in Git Bash
- `./publish.sh --file "content/posts/文章名.md"`: validate the post and run a local production build in Git Bash
- `.\publish.ps1 -Check -File "content/posts/文章名.md"`: PowerShell fallback for Windows shells
- `.\publish.ps1 -File "content/posts/文章名.md"`: PowerShell fallback for Windows shells
- `.\new.ps1 文章名`: optional Windows helper for creating `content/posts/文章名.md`
...
## Contributor Notes
Treat `themes/PaperMod/` as external code. For site behavior changes, prefer project-level overrides in `layouts/`, `static/`, or `hugo.yaml`. `publish.sh` is the primary local publishing entrypoint, and neither script commits or pushes, so always review `git status` and stage all required files manually before publishing. If you change deployment behavior, also review `.github/workflows/`.
